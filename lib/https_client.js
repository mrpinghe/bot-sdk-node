...
module.exports = class HttpsClient {
  ...
  // cb(status, obj), status = 1 for success, 0 for parsing failure, -1 for error. 
  // obj is JSON on 1, raw resp on 0, exception obj on -1
  requestForJSON(options, postBody, cb) {
    ...
    var req = https.request(options, function(res) {
        ...
        res.on("end", function() {
          try {
            ...
            console.log(`Parsing error ${e} for ${str}`);
            cb(0, str);
          }
        });
    });
    ...
    req.on("error", function(e) {
      console.log(`Request failed. Error: ${e.message}`);
      cb(-1, e);
    });

    if (options.method != null && (options.method == 'POST' || options.method == 'PUT')) {
      req.write(postBody);
    }
    req.end();
